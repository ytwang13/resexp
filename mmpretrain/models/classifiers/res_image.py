...
class resImageClassifier(BaseClassifier):
    ...
    def __init__(self,
        ...
        super(resImageClassifier, self).__init__(
    ...
    def method_init(self):
        self.is_old = True
        self.clbackbone = copy.deepcopy(self.backbone)
        self.clneck     = copy.deepcopy(self.neck)
        self.clhead     = copy.deepcopy(self.head)
        self.clbackbone = self.freeze(self.clbackbone)
        self.clneck     = self.freeze(self.clneck)
        self.clhead     = self.freeze(self.clhead)
    ...
    def foward_old(self, inputs):
        x = self.clbackbone(inputs)
        x = self.clneck(x)
        x = self.clhead(x)
        return x
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        feats = self.extract_feat(inputs)
        if self.is_old:
            old_logits = self.foward_old(inputs)
            return self.head.loss(feats, data_samples, old_logits)
